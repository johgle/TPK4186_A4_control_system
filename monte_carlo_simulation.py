...
class MonteCarloSimulation:
    ...
    # Function to generate a random workload for lanes
    def generate_workload(self):
        return random.random() #float between 0 and 1
    
    # Function to calculate the actual duratioin of tasks
    def calculate_actual_duration(self, min, exp, max):
        return random.triangular(min, exp, max)
    
    # Function to calculate durations for project nodes
    def calculate_durations(self):
        # Set workload for each lane and task, as well as expected_duration for each task
        ...
        # Calculate and set actual_duration for each task
        for task in self.project.get_tasks():
            ...
            for node in nodes:
                if node.get_start_time() == -1: #the node has not been updated.
                    ...
                    if all(con.source_node.get_start_time() != -1 for con in in_constraints): #all presuccessors have been updated
                        ...
                        #Set start time
                        if len(in_constraints) != 0: #we have presuccesors, not start gate!
                        ...
                        elif isinstance(node, Gate): # Gates has no "actual duration". Thus = 0
                            actual_time = 0
    ...
    # Function to generate testing and training set of data
